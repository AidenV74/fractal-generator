...
fractal, mode, iterations, start, generate = None, None, None, None, None
WIDTH, HEIGHT = 1600, 900  # 1600 x 900 by default
...
def apply_rules(axiom, char_1, rule_1, char_2, rule_2):
    return "".join(
        [
            rule_1 if char == char_1 else rule_2 if char == char_2 else char
            for char in axiom
        ]
    )
...
def main_menu():
    ...
    global fractal, mode, iterations, start, generate
    ...
    while not done:
        for event in pg.event.get():
            ...
            for button in start_button:
                button.handle_event(event, start_button, "start")
        ...
        if screen_state == "fractal draw":
            ...
            if event.type == pg.MOUSEBUTTONDOWN and back_button.collidepoint(
                (mouse_pos)
            ):
                time.sleep(0.1)
...
class Button:
    ...
    def handle_event(self, event, buttons, variable):
        ...
        global fractal, mode, start, generate
